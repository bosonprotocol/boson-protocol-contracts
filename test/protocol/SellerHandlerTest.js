...
describe("SellerHandlerFacet", function () {
  ...
  beforeEach(async function () {
    ...
    context("📋 Seller Methods", async function () {
        ...
        expectedCloneAddress = calculateContractAddress(sellerHandler.address, "1");
        ...
        context("👉 createSeller()", async function () {
            it("should emit a SellerCreated event when auth token is empty", async function () {
                ...
                const tx = await sellerHandler.connect(rando).createSeller(seller, contractURI, emptyAuthToken);

                await expect(tx)
                .to.emit(sellerHandler, "SellerCreated")
                .withArgs(seller.id, sellerStruct, expectedCloneAddress, emptyAuthTokenStruct, rando.address);
            ...
            it("should emit a SellerCreated event when auth token is not empty", async function () {
                ...
                sellerStruct = seller.toStruct();
                const tx = await sellerHandler.connect(rando).createSeller(seller, contractURI, authToken);

                await expect(tx)
                .to.emit(sellerHandler, "SellerCreated")
                .withArgs(seller.id, sellerStruct, expectedCloneAddress, authTokenStruct, rando.address);
            ...
            it("should update state when authToken is empty", async function () {
                // Create a seller
                await sellerHandler.connect(rando).createSeller(seller, contractURI, emptyAuthToken);

                // Get the seller as a struct
                [, sellerStruct, emptyAuthTokenStruct] = await sellerHandler.connect(rando).getSeller(id);
            ...
            it("should update state when authToken is not empty", async function () {
                ...
                // Create a seller
                await sellerHandler.connect(rando).createSeller(seller, contractURI, authToken);

                // Get the seller as a struct
                [, sellerStruct, authTokenStruct] = await sellerHandler.connect(rando).getSeller(id);
            ...
            it("should ignore any provided id and assign the next available", async function () {
                ...
                // Create a seller, testing for the event
                await expect(sellerHandler.connect(rando).createSeller(seller, contractURI, emptyAuthToken))
                .to.emit(sellerHandler, "SellerCreated")
                .withArgs(nextAccountId, sellerStruct, expectedCloneAddress, emptyAuthTokenStruct, rando.address);

                // wrong seller id should not exist
                [exists] = await sellerHandler.connect(rando).getSeller(seller.id);
                expect(exists).to.be.false;

                // next seller id should exist
                [exists] = await sellerHandler.connect(rando).getSeller(nextAccountId);
            ...
            it("should be possible to use non-unique treasury address", async function () {
                ...
                // expected address of the first clone
                expectedCloneAddress = calculateContractAddress(sellerHandler.address, "2");
            ...
            it("every seller should get a different clone address", async function () {
                ...
                // Create a seller, testing for the event
                await expect(sellerHandler.connect(other1).createSeller(seller, contractURI, emptyAuthToken))
                .to.emit(sellerHandler, "SellerCreated")
            ...
            it("should be possible to create a seller with same auth token id but different type", async function () {
                ...
                // Create a seller, testing for the event
                await expect(sellerHandler.connect(rando).createSeller(seller, contractURI, authToken))
                .to.emit(sellerHandler, "SellerCreated")
            ...
            context("💔 Revert Reasons", async function () {
                ...
                await expect(sellerHandler.connect(rando).createSeller(seller, contractURI, emptyAuthToken)).to.revertedWith(
                ...
                // Attempt to Create a seller, expecting revert
                await expect(sellerHandler.connect(rando).createSeller(seller, contractURI, authToken)).to.revertedWith(
        ...
        context("👉 getSeller()", async function () {
            beforeEach(async function () {
                ...
                // Create a seller
                await sellerHandler.connect(admin).createSeller(seller, contractURI, authToken);
                ...
                await sellerHandler.connect(rando).createSeller(seller2, contractURI, emptyAuthToken);
            });

            it("should return true for exists if seller is found", async function () {
                // Get the exists flag
                [exists] = await sellerHandler.connect(rando).getSeller(id);
            ...
            it("should return false for exists if seller is not found", async function () {
                // Get the exists flag
                [exists] = await sellerHandler.connect(rando).getSeller(invalidAccountId);
            ...
            it("should return the details of the correct seller as a struct if found", async function () {
                // Get the seller as a struct
                [, sellerStruct, authTokenStruct] = await sellerHandler.connect(rando).getSeller("1");
            ...
            it("should return the details of the correct seller if seller has empty authToken", async function () {
                // Get the seller as a struct
                [, sellerStruct, emptyAuthTokenStruct] = await sellerHandler.connect(rando).getSeller("2");
        ...
        context("👉 getSellerByAddress()", async function () {
            ...
            it("should return the correct seller when searching on operator address", async function () {
                [exists, sellerStruct, authTokenStruct] = await sellerHandler
                .connect(rando)
            ...
            it("should return the correct seller when searching on admin address", async function () {
                [exists, sellerStruct, emptyAuthTokenStruct] = await sellerHandler
                .connect(rando)
            ...
            it("should return the correct seller when searching on clerk address", async function () {
                [exists, sellerStruct, authTokenStruct] = await sellerHandler.connect(rando).getSellerByAddress(clerk.address);
        ...
        context("👉 getSellerByAuthToken()", async function () {
            beforeEach(async function () {
                ...
                await sellerHandler.connect(rando).createSeller(seller3, contractURI, authToken2);
            });

            it("should return the correct seller when searching on valid auth token", async function () {
                //Search on authToken
                [exists, sellerStruct, authTokenStruct] = await sellerHandler.connect(rando).getSellerByAuthToken(authToken);
                ...
                [exists, sellerStruct, authTokenStruct] = await sellerHandler.connect(rando).getSellerByAuthToken(authToken2);
            ...
            it("should return the correct seller when two sellers have same auth token Id but different auth token type", async function () {
                ...
                await sellerHandler.connect(rando).createSeller(seller4, contractURI, authToken3);
                ...
                [exists, sellerStruct, authTokenStruct] = await sellerHandler.connect(rando).getSellerByAuthToken(authToken3);
        ...
        context("👉 updateSeller()", async function () {
            ...
            it("should emit a SellerUpdated and OwnershipTransferred event with correct values if values change", async function () {
                ...
                const tx = await sellerHandler.connect(admin).updateSeller(seller, authToken);

                // Update a seller, testing for the event
                await expect(tx)
                .to.emit(sellerHandler, "SellerUpdated")
            ...
            it("should emit a SellerUpdated and OwnershipTransferred event with correct values if values stay the same", async function () {
                const tx = await sellerHandler.connect(admin).updateSeller(seller, emptyAuthToken);
            ...
            it("should update state of all fields exceipt Id", async function () {
                ...
                // Update a seller
                await sellerHandler.connect(admin).updateSeller(seller, authToken);

                // Get the seller as a struct
                [, sellerStruct, authTokenStruct] = await sellerHandler.connect(rando).getSeller(seller.id);
                ...
                [exists] = await sellerHandler.connect(rando).getSellerByAddress(admin.address);
                expect(exists).to.be.false;

                [exists] = await sellerHandler.connect(rando).getSellerByAddress(clerk.address);
                expect(exists).to.be.false;

                //Check that new addresses are mapped. We don't map the treasury address.
                [exists] = await sellerHandler.connect(rando).getSellerByAddress(seller.operator);
                expect(exists).to.be.true;

                //Zero address -- should return false
                [exists] = await sellerHandler.connect(rando).getSellerByAddress(seller.admin);
                expect(exists).to.be.false;

                [exists] = await sellerHandler.connect(rando).getSellerByAddress(seller.clerk);
                expect(exists).to.be.true;
            ...
            it("should update state from auth token to empty auth token", async function () {
                ...
                await sellerHandler.connect(rando).createSeller(seller2, contractURI, authToken);
                ...
                // Update seller
                await sellerHandler.connect(authTokenOwner).updateSeller(seller2, emptyAuthToken);

                // Get the seller as a struct
                [, sellerStruct, authTokenStruct] = await sellerHandler.connect(rando).getSeller(seller2.id);
                ...
                //Check that old addresses are no longer mapped. We don't map the treasury address.
                [exists] = await sellerHandler.connect(rando).getSellerByAddress(other1.address);
                expect(exists).to.be.false;

                [exists] = await sellerHandler.connect(rando).getSellerByAddress(ethers.constants.AddressZero);
                expect(exists).to.be.false;

                [exists] = await sellerHandler.connect(rando).getSellerByAddress(other3.address);
                expect(exists).to.be.false;

                //Check that new addresses are mapped. We don't map the treasury address.
                [exists] = await sellerHandler.connect(rando).getSellerByAddress(seller2.operator);
                expect(exists).to.be.true;

                [exists] = await sellerHandler.connect(rando).getSellerByAddress(seller2.admin);
                expect(exists).to.be.true;

                [exists] = await sellerHandler.connect(rando).getSellerByAddress(seller2.clerk);
                expect(exists).to.be.true;
            ...
            it("should update state from auth token to new auth token", async function () {
                ...
                // Update seller
                await sellerHandler.connect(authTokenOwner).updateSeller(seller2, authToken2);
            ...
            it("should update state correctly if values are the same", async function () {
                // Update a seller
                await sellerHandler.connect(admin).updateSeller(seller, emptyAuthToken);
            ...
            it("should update the correct seller", async function () {
                ...
                //Create seller2
                await sellerHandler.connect(rando).createSeller(seller2, contractURI, authToken2);
            ...
            it("should be able to only update with new admin address", async function () {
                ...
                await expect(sellerHandler.connect(admin).updateSeller(seller, emptyAuthToken))
                .to.emit(sellerHandler, "SellerUpdated")
                .withArgs(seller.id, sellerStruct, emptyAuthTokenStruct, admin.address);
                ...
                // Update seller, testing for the event
                await expect(sellerHandler.connect(other2).updateSeller(seller, emptyAuthToken))
                .to.emit(sellerHandler, "SellerUpdated")
                ...
                await expect(sellerHandler.connect(admin).updateSeller(seller, emptyAuthToken)).to.revertedWith(
            ...
            it("should be able to only update with new auth token", async function () {
                ...
                // Update seller, testing for the event
                await expect(sellerHandler.connect(admin).updateSeller(seller, authToken))
                .to.emit(sellerHandler, "SellerUpdated")
                .withArgs(seller.id, sellerStruct, authTokenStruct, admin.address);
                ...
                // Update seller, testing for the event
                await expect(sellerHandler.connect(authTokenOwner).updateSeller(seller, authToken))
                .to.emit(sellerHandler, "SellerUpdated")
                .withArgs(seller.id, sellerStruct, authTokenStruct, authTokenOwner.address);

                // Attempt to update the seller with original admin address, expecting revert
                await expect(sellerHandler.connect(admin).updateSeller(seller, authToken)).to.revertedWith(
                RevertReasons.NOT_ADMIN
                );
            });
            ...
            context("💔 Revert Reasons", async function () {
                ...
                // Attempt to update the seller, expecting revert
                await expect(sellerHandler.connect(operator).updateSeller(seller, emptyAuthToken)).to.revertedWith(
                    RevertReasons.NOT_ADMIN
                );
                });
                ...
                await expect(sellerHandler.connect(other2).updateSeller(seller, emptyAuthToken)).to.revertedWith(
                    RevertReasons.SELLER_ADDRESS_MUST_BE_UNIQUE
                );
                ...
                await expect(sellerHandler.connect(other2).updateSeller(seller2, authToken)).to.revertedWith(
                    RevertReasons.AUTH_TOKEN_MUST_BE_UNIQUE
                );
                });
                ...
                seller2.opeator = other5.address;

                //Mint the token to a different address
                await mockAuthERC721Contract2.connect(other8).mint(0, 2);

                // Attempt to update seller2 for token that seller doesn't own
                await expect(sellerHandler.connect(authTokenOwner).updateSeller(seller2, authToken2)).to.revertedWith(
                    RevertReasons.NOT_ADMIN
                );
                });
