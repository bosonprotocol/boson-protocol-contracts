      ...
      context("Sell order", async function () {
        context("General actions", async function () {
          ...
          it("should emit a BuyerCommitted event", async function () {
            // Sequential commit to offer, retrieving the event
            await expect(
              exchangeHandler.connect(reseller).sequentialCommitToOffer(buyer2.address, exchangeId, priceDiscovery)
            )
              .to.emit(exchangeHandler, "BuyerCommitted")
          ...
          it("should update state", async function () {
            ...
            // Sequential commit to offer
            await exchangeHandler.connect(reseller).sequentialCommitToOffer(buyer2.address, exchangeId, priceDiscovery);
          ...
          context("💔 Revert Reasons", async function () {
            ...
            it("Only seller can call, if direction is Sell", async function () {
              // Sequential commit to offer, retrieving the event
              await expect(
                exchangeHandler.connect(rando).sequentialCommitToOffer(buyer2.address, exchangeId, priceDiscovery)
