    ...
    context("👉 createOffer()", async function () {
      it("should emit an OfferCreated event", async function () {
        ...
        await expect(
          offerHandler
            .connect(assistant)
            .createOffer(offer, offerDates, offerDurations, { disputeResolverId: disputeResolver.id, mutualizerAddress: ZeroAddress }, agentId, offerFeeLimit)
        )
        ...
        context("💔 Revert Reasons", async function () {
          ...
          it("Sum of agent fee amount and protocol fee amount should be <= than the protocol wide offer fee limit", async function () {
            ...
            await expect(
              offerHandler
                .connect(assistant)
                .createOffer(offer, offerDates, offerDurations, { disputeResolverId: disputeResolver.id, mutualizerAddress: ZeroAddress }, agent.id, offerFeeLimit)
  ...
  context("📋 Offer Handler Methods - BATCH", async function () {
    ...
    beforeEach(async function () {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        // Create DRParameters with disputeResolverId and ZeroAddress for mutualizerAddress
        drParameters.push({
      ...
      disputeResolutionTermsList[4] = new DisputeResolutionTerms("0", "0", "0", "0", ZeroAddress);
    ...
    context("👉 createOfferBatch()", async function () {
      ...
      context("💔 Revert Reasons", async function () {
        ...
        it("Number of dispute resolvers does not match the number of offers", async function () {
          ...
          // Make dispute durations shorter
          drParameters = drParameters.slice(0, -2);
