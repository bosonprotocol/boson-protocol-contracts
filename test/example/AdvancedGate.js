...
const { getEvent, getFacetsWithArgs, deriveTokenId } = require("../util/utils");
...
describe("SnapshotGate", function () {
  ...
  let deployer,
    ...
    holder4,
    holder5;
  let protocolDiamond, accessController, accountHandler, offerHandler, exchangeHandler, priceDiscoveryHandler;
  ...
  beforeEach(async function () {
    ...
    const bpdFactory = await ethers.getContractFactory("BosonPriceDiscovery");
    bpd = await bpdFactory.deploy(await weth.getAddress(), await protocolDiamond.getAddress());
    ...
    const protocolConfig = [
      // Protocol addresses
      {
        ...
        priceDiscovery: await bpd.getAddress(),
      },
      // Protocol limits
      {
    ...
    facetsToDeploy["PriceDiscoveryHandlerFacet"].constructorArgs[0] = await weth.getAddress();
    ...
    priceDiscoveryHandler = await getContractAt("IBosonPriceDiscoveryHandler", await protocolDiamond.getAddress());
    ...
    [snapshotGate] = await deploySnapshotGateExample(
      ["SnapshotGateToken", "SGT", await protocolDiamond.getAddress(), sellerId, await weth.getAddress()],
    ...
    for (let holder of holders) {
      ...
      await foreign20.connect(holder).approve(await priceDiscoveryHandler.getAddress(), amountToMint);

      await weth.connect(holder).deposit({ value: amountToMint });
      await weth.connect(holder).approve(await priceDiscoveryHandler.getAddress(), amountToMint);
    ...
    const [expectedCloneAddress] = await accountHandler.getSellersCollections(seller.id);
    ...
    for (let j = 0; j < 2; j++) {
      for (let i = 1; i <= snapshotTokenCount; i++) {
        ...
        exchangeId[offerId] = await exchangeHandler.getNextExchangeId();

        await offerHandler.connect(assistant).reserveRange(offerId, offer.quantityAvailable, assistant.address);
        await bosonVoucher.connect(assistant).preMint(offerId, offer.quantityAvailable);

        await bosonVoucher.connect(assistant).setApprovalForAll(await snapshotGate.getAddress(), true);
      }
    }
  ...
  context("📋 SnapshotGate Methods", async function () {
    ...
    context("👉 commitToGatedOffer()", async function () {
      it("should emit a SnapshotTokenCommitted event when price is in native token", async function () {
        ...
        const bosonTokenId = deriveTokenId(offerId, exchangeId[offerId]);

        const priceDiscoveryContractAddress = await snapshotGate.getAddress();
        const priceDiscoveryData = snapshotGate.interface.encodeFunctionData("commitToGatedOffer", [
          entry.owner,
          offerId,
          entry.tokenId,
          bosonTokenId,
        ]);
        let priceDiscovery = new PriceDiscovery(
          price,
          Side.Ask,
        ...
        // Commit to the offer
        await expect(
          priceDiscoveryHandler.connect(holder).commitToPriceDiscoveryOffer(entry.owner, bosonTokenId, priceDiscovery)
        )
          .to.emit(snapshotGate, "SnapshotTokenCommitted")
          .withArgs(entry.owner, offerId, entry.tokenId, await bpd.getAddress());
      });
      ...
      it("should emit a SnapshotTokenCommitted event when price is in ERC20 token", async function () {
        ...
        await foreign20.connect(assistant).approve(await priceDiscoveryHandler.getAddress(), price);
