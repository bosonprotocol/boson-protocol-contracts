 ...
 */
contract AdvancedGate is BosonTypes, Ownable, ERC721 {
    ...
    function commitToGatedOffer(
        ...
        (address voucherAddress, ) = IBosonSellerHandler(protocol).getSellersCollections(offer.sellerId);
        (, BosonTypes.Seller memory seller, ) = IBosonSellerHandler(protocol).getSeller(offer.sellerId);
        ...
        ERC721(voucherAddress).safeTransferFrom(seller.assistant, msg.sender, _bosonVoucherId);
        transferFundsToToSeller(offer.exchangeToken, offer.price, seller.assistant);
     ...
     */
    function transferFundsToToSeller(address _tokenAddress, uint256 _amount, address _seller) internal {
        if (_amount > 0) {
            if (_tokenAddress == address(0)) _tokenAddress = WETH;
            // Check the allowance
            ...
            IERC20(_tokenAddress).safeTransferFrom(msg.sender, _seller, _amount);
