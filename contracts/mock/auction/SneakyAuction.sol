...
contract SneakyAuction is ISneakyAuctionErrors, ReentrancyGuard {
    ...
    function createAuction(
        ...
        emit AuctionCreated(tokenContract, tokenId, msg.sender, bidPeriod, revealPeriod, reservePrice * BID_BASE_UNIT);
    }
    ...
    function revealBid(
        ...
        if (block.timestamp <= auction.endOfBiddingPeriod || block.timestamp > auction.endOfRevealPeriod) {
            revert NotInRevealPeriodError();
        }

        uint32 auctionIndex = auction.index;
        address vault = getVaultAddress(tokenContract, tokenId, auctionIndex, msg.sender, bidValue, salt);
        ...
        if (auction.collateralizationDeadlineBlockHash == bytes32(0)) {
            ...
            if (vault.balance < bidValueWei) {
                ...
                new SneakyVault{ salt: salt }(tokenContract, tokenId, auctionIndex, msg.sender, bidValue);
                isCollateralized = false;
            } else {
                auction.collateralizationDeadlineBlockHash = blockhash(block.number - 1);
                emit CollateralizationDeadlineSet(tokenContract, tokenId, auctionIndex, block.number - 1);
            }
        } else {
        ...
        if (isCollateralized) {
            ...
            emit BidRevealed(tokenContract, tokenId, vault, msg.sender, salt, bidValueWei);
        }
    }
    ...
    function endAuction(
        ...
        if (highestBidVault == address(0)) {
            ...
            new SneakyVault{ salt: highestBidSalt }(tokenContract, tokenId, auctionIndex, highestBidder, highestBid);
        }
    }
    ...
    function withdrawCollateral(
        ...
        address vaultAddress = getVaultAddress(tokenContract, tokenId, auctionIndex, msg.sender, bidValue, salt);
    ...
    function getSeller(address tokenContract, uint256 tokenId) external view returns (address seller) {
    ...
    function getSecondHighestBid(address tokenContract, uint256 tokenId) external view returns (uint256 bid) {
    ...
    function getHighestBidVault(address tokenContract, uint256 tokenId) external view returns (address vault) {
                            ...
                            abi.encodePacked(
                                ...
                                keccak256(
                                    abi.encodePacked(
                                        ...
                                        abi.encode(tokenContract, tokenId, auctionIndex, bidder, bidValue)
                                    )
                                )
                            )
                        )
    ...
    function _getProvenAccountBalance(
        ...
        return LibBalanceProof.getProvenAccountBalance(proof, blockHeaderRLP, blockHash, account);
    }
}
