...
library LibBalanceProof {
    ...
    function getProvenAccountBalance(
        ...
        bytes32 blockHash,
        address account
    ) internal pure returns (uint256 accountBalance) {
        ...
        return getProvenAccountBalance(proof, root, proofPath);
    }
    ...
    function getStateRoot(bytes memory blockHeaderRLP, bytes32 blockHash) internal pure returns (bytes32 root) {
        ...
        assembly {
            root := mload(add(blockHeaderRLP, 0x7b))
        }
    }
    ...
    function getProvenAccountBalance(
        ...
        for (; i != lastNodeIndex; ) {
            ...
            (bool isBranchNode, bytes32 childHash) = _processBranchNode(proof[i], path, pathBitIndex);
            if (isBranchNode) {
            ...
            (partialPathLength, expectedHash) = _processExtensionNode(proof[i], path, pathBitIndex);
        ...
        return _processLeafNode(proof[i], path, pathBitIndex);
    }
    ...
    function _processBranchNode(
    ...
    ) private pure returns (bool isBranchNode, bytes32 childHash) {
        if (nodeRLP.length == 0) {
            revert EmptyNode();
        }
        ...
        uint256 byte0;
        assembly {
            byte0 := byte(0, mload(add(nodeRLP, 0x20)))
        }
    ...
    function _processLeafNode(
        ...
        bytes32 path,
        uint256 pathBitIndex
    ) private pure returns (uint256 accountBalance) {
        (
    ...
    /// @return accountBalance The proven past balance of the account.
    function _decodeAccountBalance(uint256 accountStatePtr, uint256 accountStateLen)
        private
        pure
        returns (uint256 accountBalance)
    {
        ...
        uint256 byte0;
        assembly {
            byte0 := byte(0, mload(accountStatePtr))
        }
        uint256 payloadOffset;
        if (byte0 >= LIST_LONG_START) {
            unchecked {
    ...
    function _processExtensionNode(
        ...
        bytes32 path,
        uint256 pathBitIndex
    ) private pure returns (uint256 partialPathLength, bytes32 childHash) {
        uint256 encodedPathPtr;
        uint256 encodedPathLen;
        (encodedPathPtr, encodedPathLen, childHash) = _decodeExtensionNode(nodeRLP);

        uint256 flag;
        (flag, partialPathLength) = _decodePartialPath(encodedPathPtr, encodedPathLen, path, pathBitIndex);
    ...
    function _decodePartialPath(
        ...
        bytes32 path,
        uint256 pathBitIndex
    ) private pure returns (uint256 flag, uint256 partialPathLength) {
    ...
    function _rlpItemLength(uint256 memPtr) private pure returns (uint256 itemLen) {
        uint256 byte0;
        assembly {
    ...
    function _getNibble(bytes32 path, uint256 pathBitIndex) private pure returns (uint256 nibble) {
        ...
        assembly {
            nibble := shr(252, shl(pathBitIndex, path))
        }
    }
}
