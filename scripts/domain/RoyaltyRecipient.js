...
class RoyaltyRecipientList {
   ...
   */
  static fromObject(o) {
    const { royaltyRecipient } = o;
    return new RoyaltyRecipientList(royaltyRecipient.map((f) => RoyaltyRecipient.fromObject(f)));
  }
  ...
  static fromStruct(struct) {
    return RoyaltyRecipientList.fromObject({
      royaltyRecipient: struct.map((royaltyRecipient) => RoyaltyRecipient.fromStruct(royaltyRecipient)),
    });
  }
   ...
   */
  toStruct() {
    return this.royaltyRecipient.map((f) => f.toStruct());
  }
  ...
  royaltyRecipientIsValid() {
    let valid = false;
    let { royaltyRecipient } = this;
    try {
      valid =
        Array.isArray(royaltyRecipient) &&
        royaltyRecipient.reduce(
