...
class Facet {
    ...
    facetAddressIsValid() {
        ...
        try {
            valid = (
                eip55.verify(eip55.encode(facetAddress))
            )
        } catch(e){}
        return valid;
    }
        ...
        try {
            valid = (
                ...
                functionSelectors.filter(
                    selector => (
                        ethers.BigNumber.from(selector).gte("0") &&
                        ethers.BigNumber.from(selector).lte("4294967295") // max bytes4 value
                    )
    ...
    isValid() {
        return (
            this.facetAddressIsValid() &&
            this.functionSelectorsIsValid()
        );
    };
